        ...
        for (int varLigne = -2; varLigne <= 2; varLigne++ ) {
            for (int varColonne = -2; varColonne <= +2; varColonne++) {
                if(Math.abs(varLigne) + Math.abs(varColonne) == 3){
                    ...
                    if (isCoordonneesExistent(destination) && (echiquier[ligneDestination][colonneDestination] == null || echiquier[ligneDestination][colonneDestination].getCouleur() != this.getCouleur()))
